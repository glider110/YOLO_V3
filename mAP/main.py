...
GT_PATH = os.path.join(os.getcwd(), 'input_obs', 'ground-truth')            #合成路径
DR_PATH = os.path.join(os.getcwd(), 'input_obs', 'detection-results')
# if there are no images then no animation can be shown
IMG_PATH = os.path.join(os.getcwd(), 'input_obs', 'images-optional')
...
for txt_file in ground_truth_files_list:
    ...
    for line in lines_list:
        try:
            ...
            error_msg += "\n\nIf yoou have a <class_name> with spaces between words you should remove them\n"
...
"""
 detection-results
     ...
     在所有的预测文件中寻找每一类的所有预测框，并把每一类在所有预测文件中的预测框结果保存在各自的json文件中
"""
...
for class_index, class_name in enumerate(gt_classes):
    bounding_boxes = []
    for txt_file in dr_files_list:          #循环每一个检测结果文件
        #print(txt_file)
        ...
        if class_index == 0:
            if not os.path.exists(temp_path):
                ...
                error_msg += "(You can avoid this error message by running extra/intersect-gt-and-dr.py)"
                error(error_msg)
        lines = file_lines_to_list(txt_file)                #把检测结果文件的每一行存放到一个列表中
        for line in lines:                  #循环每个检测文件中的所有预测框
            try:
