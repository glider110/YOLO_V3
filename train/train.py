...
def fit_ont_epoch(net,yolo_losses,epoch,epoch_size,epoch_size_val,gen,genval,Epoch,cuda):
    ...
    with tqdm(total=epoch_size,desc=f'Epoch {epoch + 1}/{Epoch}',postfix=dict,mininterval=0.3) as pbar:
        for iteration, batch in enumerate(gen):
            ...
            pbar.set_postfix(**{'total_loss': total_loss.item() / (iteration + 1),       #环比误差,循环刷新一次更新一次误差,不是单次一个batch
                                'lr'        : get_lr(optimizer),
                                'step/s'    : waste_time})
            pbar.update(1)
    ...
    # torch.save(model.state_dict(), model_dir[0]+'Epoch%d-Total_Loss%.4f-Val_Loss%.4f.pth'%((epoch+1),total_loss/(epoch_size+1),val_loss/(epoch_size_val+1)))
    if (epoch + 1)%5==0:
        torch.save(model.state_dict(), model_dir[0] + 'Epoch_%d.pth' % (epoch + 1))
    #loss文件的命名为训练的起始时间
    with open(model_dir[0]+date[0] +'_loss.txt', 'a+') as guo:
...
if __name__ == "__main__":
    #模型保存目录
    prefix = '/mnt/HardDisk2T/'
    date.append(time.strftime("%Y_%m_%d-%H_%M", time.localtime()))
    model_dir.append(prefix + date[0] + '/')
    if not os.path.exists(model_dir[0]):
        os.makedirs(model_dir[0])

    # 参数初始化
    # annotation_path = '2007_train.txt'
    annotation_path = '../HomeObstacleDataset/2021_train.txt'
    ...
    pretrained_dict = torch.load("../utils/yolo_weights.pth", map_location=device)
